...
const COUNTRIES = ["Argentina", "Uruguay", "Chile", "Brasil", "Colombia", "México", "Perú", "España", "USA", "Otro"];
const EXPERIENCE_LEVELS = ["Principiante (< 1 año)", "Intermedio (1-3 años)", "Avanzado (3-7 años)", "Experto (7+ años)"];
const RISK_PROFILES = ["Conservador", "Moderado", "Agresivo", "Muy Agresivo"];
const INVEST_RANGES = ["< USD 1.000", "USD 1K-10K", "USD 10K-50K", "USD 50K-200K", "USD 200K-1M", "USD 1M+"];
const SECTORS = ["Crypto", "Acciones", "Bonos", "Forex", "Commodities", "Real Estate", "Startups", "DeFi", "Energía", "Tecnología", "Agro", "ESG", "Derivados", "Private Equity"];
const PROFESSIONS = ["Inversor independiente", "Trader", "Portfolio Manager", "Analista", "Empresario", "Developer", "Contador", "Economista", "Estudiante", "Otro"];
...
function FormInput({ label, ...props }) {
  return (
    <div style={{ marginBottom: 12 }}>
      {label && <label style={{ fontFamily: "'Outfit',sans-serif", fontSize: 11, fontWeight: 600, color: X.t2, display: "block", marginBottom: 4 }}>{label}</label>}
      <input
        {...props}
        style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: "1px solid " + X.brd, background: X.inp, color: X.t1, fontSize: 13, fontFamily: "'Outfit',sans-serif", outline: "none", boxSizing: "border-box", ...(props.style || {}) }}
        onFocus={(e) => { e.target.style.borderColor = X.acc; }}
        onBlur={(e) => { e.target.style.borderColor = X.brd; }}
      />
    </div>
  );
}
...
function FormSelect({ label, options, value, onChange }) {
  return (
    <div style={{ marginBottom: 12 }}>
      {label && <label style={{ fontFamily: "'Outfit',sans-serif", fontSize: 11, fontWeight: 600, color: X.t2, display: "block", marginBottom: 4 }}>{label}</label>}
      <select value={value} onChange={(e) => onChange(e.target.value)} style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: "1px solid " + X.brd, background: X.inp, color: X.t1, fontSize: 13, fontFamily: "'Outfit',sans-serif", outline: "none", boxSizing: "border-box" }}>
...
/* ═══ NOTIFICATION HOOK ═══ */
function useNotifications(prices, isSubscribed) {
      ...
      <div style={{ background: X.bg2, borderRadius: 16, border: "1px solid " + X.brd, overflow: "hidden", marginBottom: 20 }}>
        ...
        <div style={{ padding: "40px 28px 24px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 10 }}>
            <div>
              <div style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 800, fontSize: 22, color: X.t1 }}>{profile.name}</div>
              <div style={{ fontFamily: "'Outfit',sans-serif", fontSize: 13, color: X.t2 }}>{profile.profession} {profile.company ? "· " + profile.company : ""}</div>
              <div style={{ fontFamily: "'Outfit',sans-serif", fontSize: 12, color: X.t3, marginTop: 2 }}>{profile.city}, {profile.country}</div>
            </div>
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <Bdg color={X.grn}>★ PRO</Bdg>
              <ActionBtn onClick={() => setEditing(!editing)} style={{ padding: "7px 14px", fontSize: 11 }}>{editing ? "Cancelar" : "✏️ Editar"}</ActionBtn>
            </div>
          </div>
          {profile.bio && <div style={{ fontFamily: "'Outfit',sans-serif", fontSize: 13, color: X.t2, marginTop: 12, lineHeight: 1.6, maxWidth: 500 }}>{profile.bio}</div>}
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 12 }}>
            {(profile.interests || []).map((i) => <Bdg key={i} color={X.cyn}>{i}</Bdg>)}
          </div>
        </div>
      </div>
      ...
      <div style={{ background: X.bg2, borderRadius: 14, border: "1px solid " + X.grn + "33", padding: 20 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            <div style={{ fontFamily: "'Outfit',sans-serif", fontSize: 12, color: X.grn, marginTop: 2 }}>Suscripción activa · Próximo cobro: 11/05/2026</div>
          </div>
...
/* ═══ CHAT VIEW ═══ */
function ChatView({ isSubscribed, onSubscribe, prices, cryptoPrices }) {
  ...
  if (!isSubscribed) {
    return (
      <div style={{ textAlign: "center", padding: "50px 20px", background: X.bg2, borderRadius: 20, border: "1px solid " + X.brd, maxWidth: 440, margin: "0 auto" }}>
        <div style={{ fontSize: 44, marginBottom: 14 }}>💬</div>
        <h2 style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 700, fontSize: 20, color: X.t1, marginBottom: 6 }}>Salas de Chat Premium</h2>
        <p style={{ fontFamily: "'Outfit',sans-serif", color: X.t2, fontSize: 13, marginBottom: 20 }}>Conectá con inversores en tiempo real.</p>
        <ActionBtn primary onClick={onSubscribe}>Suscribirme por USD 15/mes</ActionBtn>
      </div>
    );
  }
...
/* ═══ REGISTER MODAL ═══ */
function RegisterModal({ onClose, onComplete }) {
  const [step, setStep] = useState(1);
  const total = 4;
  const [f, setF] = useState({ name: "", email: "", password: "", phone: "", country: "Argentina", city: "", profession: "", company: "", experience: "", riskProfile: "", investRange: "", interests: [], bio: "", linkedin: "", twitter: "" });
  const upd = (k, v) => setF((p) => ({ ...p, [k]: v }));
  const toggleInt = (c) => upd("interests", f.interests.includes(c) ? f.interests.filter((x) => x !== c) : [...f.interests, c]);
  const titles = ["", "Datos Personales", "Perfil Inversor", "Tus Intereses", "Sobre Vos"];

  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 14 }} onClick={onClose}>
      <div style={{ background: X.bg, border: "1px solid " + X.brd, borderRadius: 18, maxWidth: 520, width: "100%", maxHeight: "92vh", overflow: "auto", padding: 28 }} onClick={(e) => e.stopPropagation()}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
          <div style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 800, fontSize: 20, color: X.t1 }}>{titles[step]}</div>
          <button onClick={onClose} style={{ background: "none", border: "none", color: X.t3, fontSize: 20, cursor: "pointer" }}>✕</button>
        </div>
        <div style={{ fontFamily: "'Outfit',sans-serif", fontSize: 11, color: X.t3, marginBottom: 16 }}>Paso {step} de {total}</div>
        <div style={{ height: 3, background: X.brd, borderRadius: 2, marginBottom: 20, overflow: "hidden" }}>
          <div style={{ height: "100%", width: ((step / total) * 100) + "%", background: X.acc, borderRadius: 2, transition: "width 0.3s" }} />
        </div>
        {step === 1 && (
          <div>
            <FormInput label="Nombre completo *" placeholder="Tu nombre" value={f.name} onChange={(e) => upd("name", e.target.value)} />
            <FormInput label="Email *" type="email" placeholder="tu@email.com" value={f.email} onChange={(e) => upd("email", e.target.value)} />
            <FormInput label="Contraseña *" type="password" placeholder="Mínimo 8 caracteres" value={f.password} onChange={(e) => upd("password", e.target.value)} />
            <FormInput label="Teléfono" placeholder="+54 11 1234-5678" value={f.phone} onChange={(e) => upd("phone", e.target.value)} />
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 12px" }}>
              <FormSelect label="País *" options={COUNTRIES} value={f.country} onChange={(v) => upd("country", v)} />
              <FormInput label="Ciudad *" placeholder="Tu ciudad" value={f.city} onChange={(e) => upd("city", e.target.value)} />
            </div>
          </div>
        )}
        {step === 2 && (
          <div>
            <FormSelect label="Profesión *" options={PROFESSIONS} value={f.profession} onChange={(v) => upd("profession", v)} />
            <FormInput label="Empresa" placeholder="Opcional" value={f.company} onChange={(e) => upd("company", e.target.value)} />
            <FormSelect label="Experiencia *" options={EXPERIENCE_LEVELS} value={f.experience} onChange={(v) => upd("experience", v)} />
            <FormSelect label="Perfil de riesgo *" options={RISK_PROFILES} value={f.riskProfile} onChange={(v) => upd("riskProfile", v)} />
            <FormSelect label="Rango inversión *" options={INVEST_RANGES} value={f.investRange} onChange={(v) => upd("investRange", v)} />
          </div>
        )}
        {step === 3 && (
          <div>
            <div style={{ fontFamily: "'Outfit',sans-serif", fontSize: 12, color: X.t2, marginBottom: 12 }}>Seleccioná tus sectores (mín. 3):</div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 6 }}>
              {SECTORS.map((s) => <PillBtn key={s} active={f.interests.includes(s)} onClick={() => toggleInt(s)}>{s}</PillBtn>)}
            </div>
            <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 10, color: f.interests.length >= 3 ? X.grn : X.t3, marginTop: 6 }}>
              {f.interests.length} seleccionados {f.interests.length < 3 ? "(mín. 3)" : "✓"}
            </div>
          </div>
        )}
        {step === 4 && (
          <div>
            <div style={{ marginBottom: 12 }}>
              <label style={{ fontFamily: "'Outfit',sans-serif", fontSize: 11, fontWeight: 600, color: X.t2, display: "block", marginBottom: 4 }}>Bio / Descripción</label>
              <textarea placeholder="Quién sos y qué buscás..." value={f.bio} onChange={(e) => upd("bio", e.target.value)}
                style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: "1px solid " + X.brd, background: X.inp, color: X.t1, fontSize: 13, fontFamily: "'Outfit',sans-serif", outline: "none", boxSizing: "border-box", resize: "vertical", minHeight: 70 }} />
            </div>
            <FormInput label="LinkedIn" placeholder="https://linkedin.com/in/..." value={f.linkedin} onChange={(e) => upd("linkedin", e.target.value)} />
            <FormInput label="Twitter / X" placeholder="@usuario" value={f.twitter} onChange={(e) => upd("twitter", e.target.value)} />
          </div>
        )}
        <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
          {step > 1 && <ActionBtn onClick={() => setStep((s) => s - 1)} style={{ flex: 1 }}>← Atrás</ActionBtn>}
          {step < total
            ? <ActionBtn primary onClick={() => setStep((s) => s + 1)} style={{ flex: 2 }}>Continuar →</ActionBtn>
            : <ActionBtn primary onClick={() => onComplete(f)} style={{ flex: 2 }}>✓ Crear perfil</ActionBtn>}
        </div>
      </div>
    </div>
  );
...
export default function FinPulseApp() {
  ...
  const { notifications, unreadCount, markRead, markAllRead } = useNotifications(prices, isSub);
  ...
  const handleRegComplete = (formData) => { setProfile(formData); setUserName(formData.name); setShowReg(false); setShowPay(true); };
  const handlePaySuccess = () => { setIsSub(true); setShowPay(false); };
  ...
  if (isSub) navItems.push({ id: "profile", label: "Mi Perfil", icon: "👤" });
    ...
    <div style={{ background: X.bg, minHeight: "100vh", color: X.t1, fontFamily: "'Outfit',sans-serif" }}>
      ...
      <header style={{ padding: "12px 18px", borderBottom: "1px solid " + X.brd, display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, background: X.bg, zIndex: 100, flexWrap: "wrap", gap: 8 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {isSub ? (
            <div onClick={() => setView("profile")} style={{ display: "flex", alignItems: "center", gap: 7, padding: "5px 11px", borderRadius: 7, background: X.grn + "15", border: "1px solid " + X.grn + "33", cursor: "pointer" }}>
              ...
              <div><div style={{ fontSize: 11, fontWeight: 600 }}>{userName.split(" ")[0]}</div><div style={{ fontSize: 8, color: X.grn }}>★ PRO</div></div>
            </div>
          ) : (
            <ActionBtn primary onClick={() => setShowReg(true)} style={{ padding: "7px 16px", fontSize: 11 }}>Suscribirme</ActionBtn>
          )}
        </div>
      </header>
      ...
      <main style={{ padding: 18, maxWidth: 1400, margin: "0 auto" }}>
          ...
          <div>
            <div style={{ marginBottom: 18 }}>
              <h1 style={{ fontWeight: 800, fontSize: 24, letterSpacing: "-0.02em", marginBottom: 3 }}>
                {isSub ? "Hola, " + userName.split(" ")[0] : "Mercados en Tiempo Real"}
              </h1>
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(300px,1fr))", gap: 10 }}>
              {filteredNews.map((item) => {
                const locked = item.premium && !isSub;
                ...
                return (
                  <div key={item.id} style={{ background: X.bg2, borderRadius: 10, border: "1px solid " + X.brd, padding: 14, position: "relative", overflow: "hidden", cursor: "pointer", transition: "all 0.2s" }}
                    ...
                    <div style={{ fontFamily: "'Outfit',sans-serif", fontSize: 13, fontWeight: 600, color: locked ? X.t3 : X.t1, lineHeight: 1.4, filter: locked ? "blur(2px)" : "none" }}>{item.title}</div>
                    <div style={{ fontFamily: "'Outfit',sans-serif", fontSize: 9, color: X.t3, marginTop: 6 }}>Fuente: {item.src}</div>
                    {locked && (
                      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "rgba(8,12,20,0.88)", backdropFilter: "blur(4px)", borderRadius: 10 }}>
                        <div style={{ fontSize: 20, marginBottom: 3 }}>🔒</div>
                        <ActionBtn primary onClick={() => setShowReg(true)} style={{ fontSize: 10, padding: "5px 12px" }}>Suscribirme</ActionBtn>
                      </div>
                    )}
                  </div>
                );
        ...
        {view === "finmatch" && (
          <div>
            ...
            {!isSub ? (
              <div style={{ textAlign: "center", padding: "44px 18px", background: X.bg2, borderRadius: 18, border: "1px solid " + X.brd, maxWidth: 400, margin: "0 auto" }}>
                <div style={{ fontSize: 40, marginBottom: 12 }}>🔒</div>
                <p style={{ color: X.t2, fontSize: 12, marginBottom: 16 }}>Función premium exclusiva.</p>
                <ActionBtn primary onClick={() => setShowReg(true)}>Suscribirme USD 15/mes</ActionBtn>
              </div>
        ...
        {/* CHAT */}
        {view === "chat" && <ChatView isSubscribed={isSub} onSubscribe={() => setShowReg(true)} prices={prices} cryptoPrices={cryptoPrices} />}

        {/* PROFILE */}
        {view === "profile" && profile && <ProfilePage profile={profile} onUpdate={(p) => { setProfile(p); setUserName(p.name); }} />}
      </main>

      {showReg && <RegisterModal onClose={() => setShowReg(false)} onComplete={handleRegComplete} />}
